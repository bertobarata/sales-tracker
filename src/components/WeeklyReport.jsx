...
export default function WeeklyReport({ uid }) {
  ...
  return (
    <div className="card">
      ...
      <div className="extra-fields">
        ...
        {isTouch ? (
            ...
            {['contratosFechados', 'valorTotalFechos', 'pessoasSeguras'].map(key => {
              ...
              return (
                <button
                  ...
                  type="button"
                  className="field-tap"
                  aria-label={`${f.label}: ${extra[key] || '0'}. Tocar para escrever.`}
                  onClick={() => openNumpad(key)}
                >
          ...
          <div className="field-row fluid">
            ...
            <div className="field-group">
              <label htmlFor="f-valor">Valor total (€)</label>
              <input id="f-valor" type="number" min="0" value={extra.valorTotalFechos} onChange={e => setField('valorTotalFechos', e.target.value)} placeholder="0" />
            </div>
            <div className="field-group">
              <label htmlFor="f-pessoas">Pessoas seguras</label>
              <input id="f-pessoas" type="number" min="0" value={extra.pessoasSeguras} onChange={e => setField('pessoasSeguras', e.target.value)} placeholder="0" />
            </div>
          </div>
        )}
          ...
          <div className="field-row three">
            {['reunioes1aProxSemana', 'reunioes2aProxSemana', 'reunioes3aProxSemana'].map((key, i) => (
              <button
                ...
                type="button"
                className="field-tap"
                aria-label={`${i + 1}.ª reunião: ${extra[key] || '0'}. Tocar para escrever.`}
                onClick={() => openNumpad(key)}
              >
          ...
          <div className="field-row three">
            {['reunioes1aProxSemana', 'reunioes2aProxSemana', 'reunioes3aProxSemana'].map((key, i) => (
              <div key={key} className="field-group">
                <label htmlFor={`f-${key}`}>{i + 1}.ª reunião</label>
                <input id={`f-${key}`} type="number" min="0" value={extra[key]} onChange={e => setField(key, e.target.value)} placeholder="0" />
              </div>
            ))}
          </div>
        )}
      ...
      <h3 id="preview-heading">Pré-visualização</h3>
      <div className="preview-box">
        <pre aria-labelledby="preview-heading">{previewText}</pre>
      </div>
      ...
      {activeField !== null && (
        <Sheet
          title={EXTRA_FIELDS.find(f => f.key === activeField)?.label}
